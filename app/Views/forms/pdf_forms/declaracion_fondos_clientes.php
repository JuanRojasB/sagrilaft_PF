...
$pre_tipo_doc   = htmlspecialchars($fd['representante_tipo_doc'] ?? '');
...
$pre_patrimonio = htmlspecialchars($fd['patrimonio'] ?? '');
?>
...
<div class="form-section">
    ...
    <div class="section-content">
        ...
        <div class="fr c22">
            ...
            <div class="fv"><input type="text" name="calidad" required placeholder="Cliente / Representante legal" oninvalid="this.setCustomValidity('Requerido')" oninput="this.setCustomValidity('')"></div>
        </div>
        <div class="fr c22">
            <div class="fl">EMPRESA:</div>
            <div class="fv"><input type="text" name="empresa" required value="<?= $pre_nombre ?>" placeholder="Nombre de la empresa o persona natural" oninvalid="this.setCustomValidity('Requerido')" oninput="this.setCustomValidity('')"></div>
            <div class="fl">NIT / CC EMPRESA:</div>
            <div class="fv"><input type="text" name="nit_empresa" required value="<?= $pre_documento ?>" placeholder="NIT / CC" oninvalid="this.setCustomValidity('Requerido')" oninput="this.setCustomValidity('')"></div>
        </div>
        <div class="fr cfull">
        ...
        <div class="fr cfull">
            <div class="fv" style="border-right:none; padding:10px 14px; font-size:11px; color:var(--text-secondary);">
                1. Que los recursos económicos que poseo y que utilizo para realizar transacciones comerciales con POLLO FIESTA S.A. provienen de:
            </div>
        </div>
        <div class="fr cfull">
        ...
        <div class="fr cfull">
            <div class="fv" style="border-right:none; padding:10px 14px; font-size:11px; color:var(--text-secondary); line-height:1.7;">
                ...
                3. Que NO he sido condenado(a) en Colombia o en el exterior por delitos relacionados con narcotráfico, lavado de activos o financiación del terrorismo.<br>
                4. Que la información aquí suministrada es veraz y verificable, y me comprometo a actualizarla anualmente.<br>
                5. Que autorizo a POLLO FIESTA S.A. para verificar la información aquí contenida.
            </div>
        </div>
    </div>
</div>
...
<div class="form-section">
    ...
    <div class="section-content">
        ...
        <div class="fr c1">
            ...
            <div class="fv"><input type="text" name="periodo_pep" placeholder="Desde - Hasta"></div>
        </div>
        <div class="fr c1">
        ...
        <div class="fr c1">
            <div class="fl">¿VÍNCULOS COMERCIALES CON PEP?</div>
            <div class="fv">
                <label><input type="radio" name="vinculo_pep" value="si" required> SÍ</label>
                <label><input type="radio" name="vinculo_pep" value="no" checked> NO</label>
            </div>
        </div>
        <div class="fr c1">
            <div class="fl">SI SÍ, ESPECIFIQUE:</div>
            <div class="fv"><input type="text" name="vinculo_pep_detalle" placeholder="Describa el vínculo"></div>
        </div>
    </div>
</div>
...
<div class="form-section">
    ...
    <div class="section-content">
        ...
        <div class="fr c1">
            ...
            <div class="fv"><input type="number" name="patrimonio_neto" required step="0.01" min="0" value="<?= $pre_patrimonio ?>" placeholder="0.00" oninvalid="this.setCustomValidity('Requerido')" oninput="this.setCustomValidity('')"></div>
        </div>
    </div>
</div>
    ...
    <div class="section-content">
        ...
        <div class="fr c1">
            ...
            <div class="fv" style="flex-direction:column; align-items:flex-start; gap:8px; padding:12px;">
                <input type="hidden" id="signature_declarante_fc" name="firma_declarante">
                <img id="sig_dec_fc_preview" src="" alt="Firma" style="display:none; max-width:300px; max-height:80px; border:1px solid var(--border-secondary); padding:5px; border-radius:4px; background:white;">
                <div style="display:flex; gap:8px; flex-wrap:wrap;">
                    <button type="button" id="sig_dec_fc_add" class="btn btn-primary" onclick="sigDecFC('open')" style="display:inline-flex; align-items:center; gap:6px; font-size:12px; padding:8px 14px;">
                    ...
                    <button type="button" id="sig_dec_fc_change" class="btn btn-secondary" onclick="sigDecFC('open')" style="display:none; font-size:12px; padding:8px 14px;">Cambiar</button>
                    <button type="button" id="sig_dec_fc_clear" class="btn btn-secondary" onclick="sigDecFC('clear')" style="display:none; font-size:12px; padding:8px 14px;">Limpiar</button>
                </div>
...
<script>
let _sigDecFC = null, _sigOficFC = null;
document.addEventListener('DOMContentLoaded', () => {
    _sigDecFC = new SignatureModal({ modalId: 'sigModalDecFC', onSave: (d) => {
        document.getElementById('signature_declarante_fc').value = d;
        const p = document.getElementById('sig_dec_fc_preview'); p.src = d; p.style.display = 'block';
        document.getElementById('sig_dec_fc_add').style.display = 'none';
        document.getElementById('sig_dec_fc_change').style.display = 'inline-flex';
        document.getElementById('sig_dec_fc_clear').style.display = 'inline-flex';
        const e = document.getElementById('sig-error'); if (e) e.style.display = 'none';
    }});
    _sigOficFC = new SignatureModal({ modalId: 'sigModalOficFC', onSave: (d) => {} });
});
function sigDecFC(action) {
    if (action === 'open' && _sigDecFC) { _sigDecFC.open(); return; }
    document.getElementById('signature_declarante_fc').value = '';
    document.getElementById('sig_dec_fc_preview').style.display = 'none';
    document.getElementById('sig_dec_fc_add').style.display = 'inline-flex';
    document.getElementById('sig_dec_fc_change').style.display = 'none';
    document.getElementById('sig_dec_fc_clear').style.display = 'none';
}
</script>
