<?php
// Genera app/Services/FormPdfFiller.php limpio
$code = <<<'PHPEOF'
<?php
...
class FormPdfFiller
{
    ...
    private function fgf17(): void
    {
        ...
        $this->header('DIRECCIONAMIENTO ESTRATEGICO', 'DECLARACION ORIGEN DE FONDOS - CLIENTES', '01/01/18', '10/12/2025', '02', 'FGF-17');
        ...
        $p->MultiCell($W, 4.5, $this->e(
            ...
            ', identificada con ' . ($this->v('nit') ?: $this->v('nit_empresa')) .
            ', DECLARO BAJO LA GRAVEDAD DE JURAMENTO que los recursos que utilizo para realizar transacciones comerciales con POLLO FIESTA S.A. provienen de actividades licitas y NO de ninguna actividad ilicita contemplada en el Codigo Penal Colombiano.'
        ), 1, 'L');
        ...
        $this->row1('PERIODO:', $this->v('periodo_pep'), 24, $W - 24, $h);
        ...
        $this->labelCell('VINCULO COMERCIAL PEP?', 40, $h);
        $p->SetFont('Arial', '', 6);
        $p->Cell(8, $h, $this->v('vinculo_pep') === 'si' ? 'X' : '', 1, 0, 'C');
        $p->Cell(16, $h, 'SI', 0, 0, 'L');
        $p->Cell(8, $h, $this->v('vinculo_pep') === 'no' ? 'X' : '', 1, 0, 'C');
        $p->Cell(0, $h, 'NO', 0, 1, 'L');
        $this->row1('VINCULO PEP DETALLE:', $this->v('vinculo_pep_detalle'), 40, $W - 40, $h);
        ...
        $this->row1('PATRIMONIO NETO $', $this->money('patrimonio_neto'), 34, $W - 34, $h);

        $this->sectionTitle('OPERACIONES INTERNACIONALES', $W, $hs);
        $this->labelCell('OPERA EN MONEDA EXTRANJERA?', 52, $h);
        $p->SetFont('Arial', '', 6);
        $p->Cell(8, $h, $this->v('opera_moneda_extranjera') === 'si' ? 'X' : '', 1, 0, 'C');
        $p->Cell(16, $h, 'SI', 0, 0, 'L');
        $p->Cell(8, $h, $this->v('opera_moneda_extranjera') === 'no' ? 'X' : '', 1, 0, 'C');
        $p->Cell(0, $h, 'NO', 0, 1, 'L');
        $this->row1('PAISES DE OPERACION:', $this->v('paises_operacion'), 36, $W - 36, $h);
        $this->labelCell('CUENTAS EN EL EXTERIOR?', 44, $h);
        $p->SetFont('Arial', '', 6);
        $p->Cell(8, $h, $this->v('cuentas_exterior') === 'si' ? 'X' : '', 1, 0, 'C');
        $p->Cell(16, $h, 'SI', 0, 0, 'L');
        $p->Cell(8, $h, $this->v('cuentas_exterior') === 'no' ? 'X' : '', 1, 0, 'C');
        $p->Cell(0, $h, 'NO', 0, 1, 'L');
        $this->row1('PAIS Y BANCO:', $this->v('cuentas_exterior_detalle'), 26, $W - 26, $h);
        ...
        $p->Cell(8, $h, $this->v('consulta_ofac') === 'positiva' ? 'X' : '', 1, 0, 'C');
        $p->Cell(0, $h, 'POSITIVA', 0, 1, 'L');
        $this->labelCell('CONSULTA LISTAS NACIONALES:', 50, $h);
        $p->SetFont('Arial', '', 6);
        $p->Cell(8, $h, $this->v('consulta_listas_nacionales') === 'negativa' ? 'X' : '', 1, 0, 'C');
        $p->Cell(28, $h, 'NEGATIVA', 0, 0, 'L');
        $p->Cell(8, $h, $this->v('consulta_listas_nacionales') === 'positiva' ? 'X' : '', 1, 0, 'C');
    ...
    private function v(string $k): string
    {
        $aliases = [
            ...
            'company_name'  => ['nombre_cliente', 'nombre_proveedor', 'razon_social'],
...
file_put_contents('app/Services/FormPdfFiller.php', $code);
echo "Generado OK\n";
